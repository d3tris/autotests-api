...
class TestCourses:
    ...
    def test_get_courses(
        ...
        query = GetCoursesQuerySchema(
            user_id=function_user.response.user.id)  # Формируем параметры запроса, передавая user_id
        ...
        response_data = GetCoursesResponseSchema.model_validate_json(
            response.text)  # Десериализуем JSON-ответ в Pydantic-модель

        assert_status_code(response.status_code, HTTPStatus.OK)  # Проверяем, что код ответа 200 OK
        assert_get_courses_response(response_data,
                                    [function_course.response])  # Проверяем, что список курсов соответствует ранее созданным курсам

        validate_json_schema(response.json(),
                             response_data.model_json_schema())  # Проверяем соответствие JSON-ответа схеме
